    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex gap-3">
                ...
                @endphp
                <form action="{{ url('/admin/news/toggle-section') }}" method="POST" class="inline">
                    @csrf
                ...
                <a href="{{ url('/admin/news/create') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ url('/admin/news/' . $item->id . '/edit') }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    <form action="{{ url('/admin/news/' . $item->id . '/toggle-publish') }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-{{ $item->is_published ? 'yellow' : 'green' }}-600 hover:text-{{ $item->is_published ? 'yellow' : 'green' }}-900">
                                            {{ $item->is_published ? 'Unpublish' : 'Publish' }}
                                        </button>
                                    </form>
                                    <form action="{{ url('/admin/news/' . $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this news?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <a href="{{ url('/admin/news/create') }}" class="mt-4 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                                    Create News
                                </a>
                            </td>
                        </tr>
