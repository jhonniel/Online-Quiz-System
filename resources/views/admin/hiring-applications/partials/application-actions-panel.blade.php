                ...
                <div class="px-6 py-6 space-y-3">
                    ...
                    @elseif($application->status == 'rejected' && auth()->user()->isAdmin())
                        <form action="{{ url('/admin/hiring-applications/' . $application->id . '/reconsider') }}" method="POST">
                            ...
                            <p class="mt-2 text-xs text-gray-500">
                                A user account will be automatically created with role "Applicant" and credentials will be sent via email with reconsideration message. After acceptance, use <strong>Schedule Interview</strong> to set interview date and details.
                            </p>
                        </form>
                    ...
                    @endif
                    @php
                        $canDeclineApplication = in_array($application->status, ['pending', 'accepted', 'interview_scheduled', 'done_interview'], true);
                        $declineDeactivatesAccount = in_array($application->status, ['accepted', 'interview_scheduled', 'done_interview'], true) && $application->user_id;
                    @endphp
                    @if($canDeclineApplication)
                        @include('admin.hiring-applications.partials.decline-application-form', [
                            'fieldSuffix' => $application->status,
                            'deactivateAccount' => $declineDeactivatesAccount,
                            'formClass' => 'mt-3 pt-3 border-t border-gray-200',
                        ])
                    @endif
