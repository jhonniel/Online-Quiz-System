...
@section('title', 'Import Starlinks')
...
<div class="px-3 sm:px-4 lg:px-6 py-4 max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-4 sm:px-6 py-5 border-b border-gray-200 bg-gray-50/50">
            <h2 class="text-lg font-semibold text-gray-900">Import Starlink devices from CSV</h2>
            <p class="text-sm text-gray-500 mt-0.5">Upload a CSV file with the same columns as the sample template. <code class="text-xs bg-gray-100 px-1 rounded">start_date</code> must be in <code class="text-xs bg-gray-100 px-1 rounded">Y-m-d</code> format (e.g. 2024-01-15).</p>
        </div>
        <div class="p-4 sm:p-6 space-y-6">
            <div class="rounded-lg border border-indigo-200 bg-indigo-50/50 p-4">
                ...
                <a href="{{ url('/admin/starlinks/import/template') }}" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    Download starlinks_import_template.csv
                </a>
            </div>

            <form action="{{ url('/admin/starlinks/import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
