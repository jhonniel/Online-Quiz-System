            ...
            <div>
                ...
                <p class="mt-1 text-sm text-indigo-100">
                    @if($isFullAccessAdmin)
                        Select a user (non-employee) to assign feature access permissions. Only full-access admins can assign permissions to non-employee users.
                    @else
    ...
    <div class="flex-1 bg-white rounded-lg shadow-sm border border-gray-200 mx-2 sm:mx-3 lg:mx-4 xl:mx-6 p-4 sm:p-6">
        @if($users->isEmpty())
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">All {{ $isFullAccessAdmin ? 'non-employee users' : 'employees' }} already have permissions assigned.</p>
                <div class="mt-6">
                    <a href="{{ route('admin.admin-permissions.index') }}"
            ...
            <form action="{{ route('admin.admin-permissions.store') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mb-4">
                        @if($isFullAccessAdmin)
                            Choose a user (non-employee) to add to the permissions management system. You can configure their feature access after adding them.
                        @else
