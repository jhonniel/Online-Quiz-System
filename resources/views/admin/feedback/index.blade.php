...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 rounded-lg shadow-sm p-6">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                <h1 class="text-2xl font-bold text-white">Feedback Management</h1>
                <p class="text-indigo-100">Review and manage user feedback to improve the system</p>
            </div>
        </div>
    </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900">{{ $stats['in_review'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="px-4 py-5 sm:p-6">
            <form method="GET" action="{{ route('admin.feedback.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="priority" id="priority" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
    ...
    <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($feedbacks as $feedback)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <div class="text-sm font-medium text-gray-900 flex items-center">
                                        ...
                                        @if($feedback->hasImage())
                                            <div class="flex items-center ml-2">
                                                <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Has attached images">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                </svg>
                                                <span class="ml-1 text-xs text-blue-600 font-medium">{{ $feedback->image_count }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="text-sm text-gray-500 truncate max-w-xs">{{ Str::limit($feedback->description, 80) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            @if($feedback->user->profile_picture)
                                                <img class="h-10 w-10 rounded-full object-cover" src="{{ Storage::url($feedback->user->profile_picture) }}" alt="{{ $feedback->user->name }}">
                                            @else
                                                <div class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center">
                                                    <span class="text-sm font-medium text-indigo-600">{{ substr($feedback->user->name, 0, 1) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $feedback->type_badge_class }}">
                                        {{ $feedback->type_icon }} {{ ucfirst(str_replace('_', ' ', $feedback->type)) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $feedback->priority_badge_class }}">
                                        {{ $feedback->priority_icon }} {{ ucfirst($feedback->priority) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $feedback->status_badge_class }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open"
                                            ...
                                            <form method="POST" action="{{ route('admin.feedback.destroy', $feedback) }}" class="block" onsubmit="return confirmFeedbackAction('delete', this)">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div class="flex items-center">
                        <p class="text-sm text-gray-700">
                            Showing
                            <span class="font-medium">{{ $feedbacks->firstItem() }}</span>
                            to
                            <span class="font-medium">{{ $feedbacks->lastItem() }}</span>
                            of
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">No feedback matches your current filters.</p>
            </div>
        @endif
    </div>
</div>
