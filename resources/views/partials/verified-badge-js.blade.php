...
(function () {
    ...
    function badgeSvg(size) {
        const s = Math.max(12, Math.min(18, Number(size) || 14));
        const gradId = 'vb-js-' + Math.random().toString(36).slice(2, 10);
        const sizeClass = s <= 12 ? 'verified-badge--xs' : (s >= 18 ? 'verified-badge--profile' : 'verified-badge--sm');
        return '<span class="verified-badge ' + sizeClass + ' inline-flex items-center flex-shrink-0" title="Verified account" role="img" aria-label="Verified account">'
            + '<svg viewBox="0 0 24 24" width="' + s + '" height="' + s + '" class="twetch-green-check" aria-hidden="true" focusable="false" style="width:' + s + 'px;height:' + s + 'px">'
    ...
    window.VerifiedBadgeUI = {
        ...
        nameHtml: function (name, verified, size) {
            ...
            return '<span class="inline-flex items-center gap-px min-w-0 max-w-full align-middle">'
                + '<span class="truncate">' + safeName + '</span>'
                + badgeSvg(size || 14).replace('verified-badge ', 'verified-badge -ml-0.5 ')
                + '</span>';
        },
