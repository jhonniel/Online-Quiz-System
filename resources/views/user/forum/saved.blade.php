...
@section('page-title', 'Saved Forum Posts')
...
<div class="h-screen flex flex-col space-y-2 overflow-hidden">
    ...
    <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 rounded-lg shadow-sm p-3 flex-shrink-0 mx-2 sm:mx-3 lg:mx-4 xl:mx-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-white">Saved Threads</h1>
                <p class="text-indigo-100 text-sm mt-1">Your bookmarked forum discussions</p>
            </div>
            <div class="text-right">
                <div class="text-2xl font-bold text-white">{{ $savedThreads->total() }}</div>
                <div class="text-indigo-100 text-sm">Saved Threads</div>
            </div>
        </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto mx-2 sm:mx-3 lg:mx-4 xl:mx-6">
        <div class="space-y-4 pb-4">
            @forelse($savedThreads as $save)
            @php $thread = $save->thread; @endphp
                    ...
                    <div class="mt-4 flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <button onclick="toggleLike({{ $thread->id }})"
                                    class="flex items-center space-x-1 px-3 py-2 text-sm font-medium rounded-md transition-colors text-gray-600 hover:text-red-600 hover:bg-red-50">
                            ...
                            <button onclick="toggleSave({{ $thread->id }})"
                                    class="flex items-center space-x-1 px-3 py-2 text-sm font-medium rounded-md transition-colors text-blue-600 bg-blue-50">
                        ...
                        <div class="flex items-center space-x-2">
                            <span class="text-xs text-gray-500">Saved {{ $save->created_at->diffForHumans() }}</span>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">No saved threads yet</h3>
                <p class="text-gray-500 mb-4">Save interesting threads to read them later.</p>
                <a href="{{ route('forum.index') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
