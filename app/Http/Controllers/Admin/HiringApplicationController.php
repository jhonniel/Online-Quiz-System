...
class HiringApplicationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Sort: for interview_scheduled filter use interview date, otherwise keep workflow + application date sorting.
        $sort = $request->input('sort', 'latest');
        $sort = in_array($sort, ['latest', 'oldest'], true) ? $sort : 'latest';

        if ($statusFilter === 'interview_scheduled') {
            if ($sort === 'oldest') {
                $query->orderByRaw('CASE WHEN interview_date IS NULL THEN 1 ELSE 0 END ASC')
    ...
    public function acceptIntern(Request $request, HiringApplication $application)
    {
        ...
        if ($ojtTotalSlots > 0) {
            ...
            if ($updatedUsedSlots > $ojtTotalSlots) {
                $overBy = $updatedUsedSlots - $ojtTotalSlots;
                $successMessage .= " Warning: OJT capacity exceeded ({$updatedUsedSlots}/{$ojtTotalSlots} used, over by {$overBy} slot".($overBy > 1 ? 's' : '').').';
            } else {
                $remainingSlots = $ojtTotalSlots - $updatedUsedSlots;
                if ($remainingSlots <= 3) {
